...
if [[ -z "$apitree_commit_message" ]]; then
    git_commit_hash=$(git --git-dir=${GITHUB_WORKSPACE} log -1 --pretty=%H -- ${apitree_api_file})
    git_commit_hash_abbrev=$(git --git-dir=${GITHUB_WORKSPACE} log -1 --pretty=%h -- ${apitree_api_file})
    git_commit_subject=$(git --git-dir=${GITHUB_WORKSPACE} log -1 --pretty=%s -- ${apitree_api_file})
